...
class _ChannelsScreenState extends ConsumerState<ChannelsScreen> {
  ...
  // True while the channel list is actively scrolling. Reachability probes (and
  // logo loads) are gated on this so a fast fling doesn't dial every host it
  // flies past — the probe only fires once scrolling settles on a row.
  bool _channelListScrolling = false;
  Timer? _channelScrollSettleTimer;
  ...
  void initState() {
    ...
    // Track channel-list scrolling so reachability probes only fire once the
    // list settles on a row (see [_channelListScrolling]).
  ...
  /// Marks the channel list as scrolling and schedules a "settled" reset. While
  /// scrolling, per-row reachability probes are suppressed; once the list has
  /// been still for a short beat, [_channelListScrolling] flips back to false
  /// and the visible rows probe their hosts.
  void _onChannelListScroll() {
    if (!_channelListScrolling && mounted) {
      setState(() => _channelListScrolling = true);
    }
    _channelScrollSettleTimer?.cancel();
    _channelScrollSettleTimer = Timer(const Duration(milliseconds: 250), () {
      if (mounted) setState(() => _channelListScrolling = false);
    });
  }
